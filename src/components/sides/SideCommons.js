    ...
    <>
      ...
      {type && (
        <SideComponents.AddressField
          label={label}
          value="address.value"
          onApplySuggestion={(s) => setSuggestion(s.data)}
          ctx={ctx}
        />
      )}
      <SideComponents.InputField
...
export function PhysicalFields({ ctx }) {
  return (
    <>
      ...
      />
      {ctx.side === "zaimodavec" && (
        <SideComponents.InputField label="E-Mail" value="email" ctx={ctx} />
      )}
...
export function JuridicalFields({ ctx }) {
  ...
  return (
    <>
      ...
      <SideComponents.StatefulCheckboxLabel
        text="Договор займа заключён с филиалом или представительством"
        initiallyCollaped={!ctx.sideData.filial?.name}
      >
        <SideComponents.InputField
